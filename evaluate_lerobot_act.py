#!/usr/bin/env python3
"""
Evaluation script for Lerobot ACT policies in LIBERO environments.

This script allows evaluation of pretrained Lerobot ACT policies on LIBERO tasks
with configurable options for rendering, task selection, and evaluation parameters.

Usage:
    python evaluate_lerobot_act.py --model_path /path/to/model --task_id 0 --n_eval 10
"""
...
def setup_libero_config(
    ...
    cfg.lifelong.algo = "LerobotACTAlgorithm"
...
def evaluate_act_policy(
    ...
    """
    Evaluate a pretrained Lerobot ACT policy on a LIBERO task.
    ...
    if verbose:
        print("=" * 60)
        print("LEROBOT ACT POLICY EVALUATION IN LIBERO")
        print("=" * 60)
    ...
    # Create algorithm with ACT policy
    if verbose:
        print("Loading ACT policy...")
    cfg.shape_meta = shape_meta  # Add shape_meta to config
    algo = LerobotACTAlgorithm(model_path, cfg, shape_meta)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate Lerobot ACT policy in LIBERO environment"
    )
    ...
    # Run evaluation
    try:
        success_rate = evaluate_act_policy(

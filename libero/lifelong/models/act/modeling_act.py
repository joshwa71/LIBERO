...
Adapted from Lerobot's ACT implementation for use within LIBERO.
Removes Lerobot dependencies and adapts for torch 1.11.0 compatibility.
"""
...
class ACTPolicy(nn.Module):
    """
    Action Chunking Transformer Policy adapted for LIBERO.
    
    Simplified version of Lerobot's ACTPolicy that can be loaded and used
    ...
    @classmethod
    def from_pretrained(cls, pretrained_path: Union[str, Path]) -> "ACTPolicy":
        """Load a pretrained ACT policy from a directory containing model files."""
        ...
        config = ACTConfig.from_json_file(config_file)
        ...
        # Load the state dict with missing normalization parameters
...
class ACT(nn.Module):
    ...
    def __init__(self, config: ACTConfig):
        ...
        if self.config.image_features:
            # Handle torchvision compatibility for older versions
            backbone_kwargs = {
                "replace_stride_with_dilation": [False, False, config.replace_final_stride_with_dilation],
                "norm_layer": FrozenBatchNorm2d,
            }
            
            # For torch 1.11.0, use 'pretrained' instead of 'weights'
...
class ACTSinusoidalPositionEmbedding2d(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        inverse_frequency = self._temperature ** (
            2 * torch.div(torch.arange(self.dimension, dtype=torch.float32, device=x.device), 2, rounding_mode='trunc') / self.dimension
        )
